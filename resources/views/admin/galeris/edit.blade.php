        ...
        <form action="{{ route('admin.galeris.update', $galeri) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="keterangan" rows="2"
                    class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-400">{{ old('keterangan', $galeri->keterangan) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Ganti Foto</label>
                <img src="{{ Storage::url($galeri->gambar) }}" class="w-full h-48 object-cover rounded mb-2">
                ...
                <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin ganti foto.</p>
